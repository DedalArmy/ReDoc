...
import fr.imt.ales.redoc.cba.dedal.extractor.ComponentClassExtractor;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public DedalArchitectureBuilder(String projectPath) throws IOException {
		...
		this.dedalArchitecture = new DedalArchitecture(projectPath);
	}
...
//	}
	...
	public DedalDiagram build(XMLFile springXMLFile) throws URISyntaxException {
		
		// Setting the DedalDiagram up
		DedalDiagram dedalDiagram = new DedalFactoryImpl().createDedalDiagram();
		...
		String baseName = springXMLFile.getPath().replace('.', '_');
		Specification spec = factory.createSpecification();
		spec.setName(baseName+"_spec");
		Configuration config = factory.createConfiguration();
		config.setName(baseName+"_config");
		dedalDiagram.getArchitectureDescriptions().add(config);
		Assembly asm = factory.createAssembly();
		asm.setInstantiates(config);
		asm.setName(baseName+"_asm");
		dedalDiagram.getArchitectureDescriptions().add(asm);

		// SpringToDedal transformation
		List<EObject> extractedFromSpring = SdslTransformer.extractDedalArtifacts(springXMLFile);
		copyInDiagram(extractedFromSpring);
//		standardizeNames(config, asm);
//
//		logger.info("XML File : " + springXMLFile.getPath());
...
//	}
	 ...
	 * @param asm
	 */
	private void copyInDiagram(List<EObject> extractedFromSpring) {
				...
				((DedalDiagram) artefact).getArchitectureDescriptions().forEach(ad -> {
					if(ad instanceof Assembly)
					{
						for(CompInstance ci : ((Assembly) ad).getAssmComponents()) {
							DedalComponentInstance compInstance;
							try {
								compInstance = new DedalComponentInstance(this.projectPath, ci, this.factory, ((Assembly) ad).getAssemblyConnections());
								this.dedalArchitecture.getAssembly().add(compInstance);
							} catch (IOException e) {
					...
					else if(ad instanceof Configuration)
					{
						for(CompClass cc : ((Configuration) ad).getConfigComponents()) {
							DedalComponentClass compClass;
							try {
								compClass = new DedalComponentClass(this.projectPath, cc, this.factory);
								this.dedalArchitecture.getConfiguration().add(compClass);
							} catch (IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
...
//			ArrayList<Class<?>> classes =  (ArrayList<Class<?>>) f.get(hierarchyBuilder.getJarLoader());
